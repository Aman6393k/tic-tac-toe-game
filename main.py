		...
		while self.jeu_encours:
			...
			for ligne in range(0,len(self.grille.grille)):
				for collonne in range(0, len(self.grille.grille)):
					...
					if self.grille.grille[ligne][collonne] == 'X':

						x_pos = (ligne,collonne)
						liste_X.append(x_pos)

					elif  self.grille.grille[ligne][collonne] == 'O':
						o_pos = (ligne,collonne)
						liste_O.append(o_pos)

			if len(liste_X) >= 3 :
				for ligne,collonne in liste_X:
					liste_lignes_X.append(ligne)
					liste_colonnes_X.append(collonne)
